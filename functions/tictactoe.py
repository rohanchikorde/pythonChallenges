# tic tac toe

def draw_board(char_list):
    """print a game board - either a number board or tic tac toe board"""
    print('\n\t   Tic-Tac-Toe')
    print('\t~~~~~~~~~~~~~~~~~')
    print('\t|| ' + char_list[0] + ' || ' + char_list[1] + ' || ' + char_list[2] + ' || ')
    print('\t~~~~~~~~~~~~~~~~~')
    print('\t|| ' + char_list[3] + ' || ' + char_list[4] + ' || ' + char_list[5] + ' || ')
    print('\t~~~~~~~~~~~~~~~~~')
    print('\t|| ' + char_list[6] + ' || ' + char_list[7] + ' || ' + char_list[8] + ' || ')
    print('\t~~~~~~~~~~~~~~~~~')
    
    
def get_player_input(player_char, char_list):
    """get a player move until it is a valid move on the board with no piece currently there."""
    while True:
        # get user input
        player_move = int(input(player_char + ': Where would you like to place you piece (1-9): '))
        if player_move > 0 and player_move < 10:
            if char_list[player_move - 1] == '_':
                return player_move
            else:
                print('\nThat spot has already been choosen. Try again!')
        else:
            print('\nThat is not a spot on the board. Try again!')


def place_char_on_board(player_char, player_move, char_list):
    """put a player character at the correct spot on the board"""
    char_list[player_move - 1] = player_char
    
    
    
def is_winner(pC, cL):
    """return a bool if a given player is winner"""
    return ((cL[0] == pC and cL[1] == pC and cL[2] == pC) or  # victory in first row 
            (cL[3] == pC and cL[4] == pC and cL[5] == pC) or  # victory in second row
            (cL[6] == pC and cL[7] == pC and cL[8] == pC) or  # victory in third row
            (cL[0] == pC and cL[3] == pC and cL[6] == pC) or  # victory in first col
            (cL[1] == pC and cL[4] == pC and cL[7] == pC) or  # victory in second col
            (cL[2] == pC and cL[5] == pC and cL[8] == pC) or  # victory in third col
            (cL[0] == pC and cL[4] == pC and cL[8] == pC) or  # victory in diag one
            (cL[2] == pC and cL[4] == pC and cL[6] == pC))     # victory in diag two

# main code
def main():
    player1 = 'X'
    player2 = '0'
    n_list = ['1', '2', '3', '4', '5', '6', '7', '8', '9']
    c_list = ['_']*9

    # draw the initial state of the board
    draw_board(n_list)
    draw_board(c_list)
    while True:
        # player 1 turn
        # get the player moves
        move = get_player_input(player1, c_list)
        # put on board
        place_char_on_board(player1, move, c_list)
        # redraw board
        draw_board(n_list)
        draw_board(c_list)
        # check to see if player 1 won or it is a tie
        if is_winner(player1, c_list):
            print('\nPlayer 1 wins')
            break
        # check if tie
        elif '_' not in c_list:
            print('\nThe game was a tie')
        
        # player 2 turns
        move = get_player_input(player2, c_list)
        #put move on board
        place_char_on_board(player2, move, c_list)
        # redraw board
        draw_board(n_list)
        draw_board(c_list)
        
        # check if player2
        if is_winner(player2, c_list):
            print('\nPlayer 2 wins')
            break
        
        
if __name__ == "__main__":
    main()
        
        


